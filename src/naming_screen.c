...
static void NamingScreen_CreateRivalIcon(void)
{
    ...
    //rivalGfxId = GetRivalAvatarGraphicsIdByStateIdAndGender(PLAYER_AVATAR_STATE_NORMAL, gSaveBlock2Ptr->playerGender ^ 1); //This would make it so that it generates rival icon based on gender rather than the predetermined overworld we created (Rival Logan(?))
    spriteId = CreateObjectGraphicsSprite(OBJ_EVENT_GFX_RIVAL, SpriteCallbackDummy, 56, 37, 0); //OBJ_EVENT_GFX_RIVAL replaced "rivalGfxId"
    gSprites[spriteId].oam.priority = 3;
...
void NameRival(void)
{
    static const u8 *const sDefaultRivalNames[2] = {gText_ExpandedPlaceholder_Brendan, gText_ExpandedPlaceholder_May}; //I think if array was made bigger, more arguments could be inserted.
